 ...
 */
@WebServlet("/createExamfileId")
public class CreateExamfileId extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CreateExamfileId() {
        super();
		 ...
		 request.setAttribute("noti", noti);
	        ...
	        request.setAttribute("examList", exam);
			  RequestDispatcher dispatcher = request.getServletContext()
		                .getRequestDispatcher("/WEB-INF/views/them-hosoid.jsp?id="+id+"&name="+name);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (errorString == null) {
            try {
            	ExamfileDAO.insertExamDetail(conn, detail);
            } catch (SQLException e) {
                e.printStackTrace();
                errorString = e.getMessage();
            }
...
//        request.setAttribute("department", department);
        ...
        if (errorString != null) {
	        ...
	        List<Noti> noti = null;
			 ...
			 request.setAttribute("noti", noti);
            RequestDispatcher dispatcher = request.getServletContext()
                    .getRequestDispatcher("/WEB-INF/views/them-hosoid.jsp?id="+userId+"&name="+name);
            dispatcher.forward(request, response);
        }
        // Nếu mọi thứ tốt đẹp.
        // Redirect (chuyển hướng) sang trang danh sách sản phẩm.
        else {
        	System.out.println("Created	 Suckshit");
